...
#     x1, x2 = get_lat(city1), get_lat(city1)
...
#     """
#     "*** YOUR CODE HERE ***"
#     pres_city = make_city('pres', lat, lon)
#     d_to_c1 = distance(pres_city, city1)
#     d_to_c2 = distance(pres_city, city2)
#     if d_to_c1 < d_to_c2:
#         get_name(city1)
#     elif d_to_c1 > d_to_c2:
#         get_name(city2)
#     else:
#         print('Draw!')
...
def put_piece(board, max_rows, column, player):
    ...
    free_row = max_rows-1
    while free_row >= 0:
        if get_piece(board, free_row, column) == '-':
            new_row = replace_elem(board[free_row], column, player)
            new_board = replace_elem(board, free_row, new_row)
            return free_row, new_board
        free_row -= 1
    return -1, board

#######################################################################
###############CROSSING DATA ABSTRACTION BARRIER#######################
#######################################################################
...
def make_move(board, max_rows, max_cols, col, player):
    ...
    """
    "*** YOUR CODE HERE ***"
    if col > max_cols or col < 0 or get_piece(board,0,col) is not '-':
        return -1, board
    else:
...
def check_win_row(board, max_rows, max_cols, num_connect, row, player):
    ...
    counter = 0
    for c in range(max_cols):
        if get_piece(board, row, c) == player:
            counter += 1
    return counter >= num_connect
...
def check_win_column(board, max_rows, max_cols, num_connect, col, player):
    ...
    counter = 0
    for r in range(max_rows):
        if get_piece(board, r, col) == player:
            counter += 1
    return counter >= num_connect
